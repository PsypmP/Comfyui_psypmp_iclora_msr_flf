...
* **v0.9.5:** Исправлен критический баг рассогласования временных координат (RoPE) при использовании MSR-видео.
  <details>
  <summary><b>🔍 Подробный технический разбор бага и его исправления</b></summary>

  #### В чём заключалась проблема?
  Механизм внимания MSR (Cross-Attention) генерирует последовательность кадров (MSR Video Sequence) для удержания деталей персонажа на протяжении всего таймлайна. Для этого на Шаге 3 вызывался метод `append_keyframe`, который регистрировал временные координаты кадров в кондиционировании и возвращал обновленный латентный тензор. Но возвращаемый латент выбрасывался (через `positive, negative, _, _ = append_keyframe(...)`), чтобы не добавлять MSR-картинки физически в ролик.

  #### Глубокая физика бага:
  В ядре LTX-Video (`comfy/ldm/lightricks/model.py`) позиционирование ключевых кадров во времени выполняется по строгому смещению с конца физического тензора латентов:
  $$\text{Срез токенов} = -K$$
  где $K$ — общее число зарегистрированных в кондиционировании ключевых кадров (включая FF, MSR и LF). 
  Если мы зарегистрировали 13 ключевых кадров (1 FF + 5 MSR-img + 6 MSR-vid + 1 LF), но реально физически приклеили к латенту только 7 кадров, ядро сэмплера принудительно перезаписывает временные координаты последних 13 токенов физического последовательности. В результате модель заходила на 6 кадров вглубь **основного генерируемого видео** (финальные пиксельные кадры 313–360) и принудительно выставляет им координату $t = 0$ (так как MSR-видео имеет нулевую отметку).

  #### Симптомы:
  В финальную секунду видео движение полностью ломалось, сэмплирование зависало или шло вспять, а переход к заблокированному Last Frame (на $t=360$) разрушался, из-за чего опция `lock_last_frame` визуально не срабатывала.

  #### Как это исправлено:
  Мы исправили это, бережно сохраняя возвращаемые латенты и маску шума из `append_keyframe` на Шаге 3 и увеличивая счетчик `num_appended` на количество кадров MSR-видео. В конце сэмплирования патч на ноду `LTXVCropGuides` чисто отрезает все 13 физических кадров, гарантируя идеальные координаты основного видео и железный замок последнего кадра.
  </details>
